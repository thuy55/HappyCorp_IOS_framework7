...
const SocialFollowerPage = () => {
    ...
    const ListFollower = () => {
        ...
        api.post("/social_follower", data, {
            ...
            if (res.data.status === "error") {
                ...
                setListFollower(res.data.follower)
    ...
    const filteredFollower = listFollower
        .filter((follower) =>
            follower.name.toLowerCase().includes(searchFollower.toLowerCase())
        )
        .slice(0, 20);
        ...
        <Page name="profile" >
            ...
            <List className='m-3'>
                ...
                <div className='d-flex justify-content-between align-items-center'>
                    <div className='fs-15 fw-bold'>Người theo dõi ({listFollower.length})</div>
                    <div className="input-group align-items-center  p-2 py-1 mt-2 rounded-pill bg-dark bg-opacity-75" style={{ width: "60%" }}>
                        <input
                            ...
                            onChange={(e) => setsearchFollower(e.target.value)} // Cập nhật state khi nhập
                        />
                        <span
                ...
                <div className='row mx-0 mt-2'>
                    {filteredFollower && filteredFollower.map((follower) => {
                        return (
                            <>
                                <div className='col-6 p-2'>
                                    <Card className=' bg-dark bg-opacity-75 rounded-4 m-0 p-3' >
                                        {follower.avatar === "no-image" ?
                                            <img src="../img/logo-small.svg" className='rounded-circle w-100' onClick={() => { profile_social(follower.uid) }}></img>
                                            :
                                            <img src={`https://beta.ellm.io/${follower.avatar}`} className='rounded-circle w-100' onClick={() => { profile_social(follower.uid) }}></img>
                                        }

                                        <div className='mt-1 fs-15 text-center'>{follower.name}</div>
                                    </Card>
                                </div>
                            </>
                        )
...
export default SocialFollowerPage;